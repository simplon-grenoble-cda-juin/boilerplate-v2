...
export default class Token {
  ...
  static create = (userId: number, jwt: string): Token => {
    ...
    const expires = new Date(created.getTime() + 12 * 60 * 60 * 1000); // +12h
  ...
  /** Re-hashe la valeur brute du token (utile pour comparer avec le hash en BDD)
   * @param raw - Version brute du token
   */
